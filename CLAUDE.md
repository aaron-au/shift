...
- **Security gates on every push (ADR-0006):** `make check` = govulncheck + gosec/staticcheck/errcheck (golangci-lint) + gitleaks + `go test -race` + fmt — identical locally (pre-push hook), and in CI. Findings are fixed or suppressed inline with justification. **`depguard` enforces the module-boundary doctrine below as a build gate** (connectors → sdk → engine; hub/runner never import sdk or the engine payload plane; engine/pkg are leaves) — a forbidden import fails `make lint`. **Before pushing, run `golangci-lint cache clean && make check`** — a stale lint cache can hide findings the pre-push hook then rejects.
