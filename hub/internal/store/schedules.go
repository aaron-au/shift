...
func (s *Store) FireDue(ctx context.Context, nextFn func(cronExpr string, after time.Time) (time.Time, error), limit int) (fired int, err error) {
	...
	for _, d := range dues {
		...
		// Never test-marked, and stated rather than defaulted: a scheduled
		// flow running on test capacity is a production flow metered wrong
		// (ADR-0048 §3).
		taskID, err := enqueueTx(ctx, tx, d.account, d.flowID, d.flowName, d.published, doc, key,
			effectiveMaxAttempts(doc, d.maxAttempts), false)
		if err != nil {
			return fired, err
		}
		if _, err := tx.Exec(ctx,
