...
//
// Why this exists (ADR-0005, ADR-0029): every task gets its own goroutine(s),
// and a v3 flow fans out concurrent branches joined by stream.Pipe. The failure
// mode that matters is not "the flow returned the wrong answer" — the flow-level
// assertions catch that — it is "the flow returned, and a branch goroutine is
// still parked on a channel nobody will ever send to". Under a long-lived
// runner that is a slow leak; the test suite is where it is cheap to notice.
//
// Why it is hand-rolled rather than go.uber.org/goleak: engine/go.mod has ZERO
// dependencies and that is a doctrine property, not an accident (CLAUDE.md —
// "engine stays stdlib-only"). The gateway holds the same line and enforces it
// with a test. A leak detector is ~150 lines of runtime.Stack parsing; taking a
// module dependency to avoid writing them would cost more than it saves.
//
// Usage — whole package, the normal case:
//
//	func TestMain(m *testing.M) { leaktest.Main(m) }
//
// With teardown that must happen before the check:
//
//	func TestMain(m *testing.M) { leaktest.Main(m, func() { os.RemoveAll(dir) }) }
//
// A single test (do NOT use in a package with t.Parallel tests — a sibling's
// goroutines are indistinguishable from a leak):
//
//	defer leaktest.Check(t)()
//
// The check is an identity diff, not a stack denylist: goroutines alive before
// the run are ignored no matter what they are, so pre-existing framework and
// package-init goroutines never register. Only goroutines *created during* the
// run and still alive at the end are reported. The small ignore list below is
// for runtime-owned goroutines that can be created lazily mid-run and are never
