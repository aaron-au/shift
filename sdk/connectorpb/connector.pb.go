...
type PullRequest struct {
	...
	// Opaque position to restart the stream from, previously emitted as a
	// Frame.checkpoint by this same action (ADR-0037 resume). Absent or empty
	// means "from the beginning" — the behaviour of every connector that does
	// not implement resumption. Neither the runner nor the hub interprets these
	// bytes; only the connector that produced them can.
	ResumeFrom    []byte `protobuf:"bytes,3,opt,name=resume_from,json=resumeFrom,proto3" json:"resume_from,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *PullRequest) GetResumeFrom() []byte {
	if x != nil {
		return x.ResumeFrom
	}
	return nil
...
type Frame struct {
	...
	// Opaque position the connector asserts is safe to resume from ONCE every
	// record up to and including this frame has been processed downstream
	// (ADR-0037). Optional; absent means "no safe position yet". The runner
	// only ever persists a checkpoint the terminal sink has confirmed —
	// persisting on source progress would skip records that were read but never
	// written, turning a resume into silent data loss.
...
func (x *Frame) GetCheckpoint() []byte {
	if x != nil {
		return x.Checkpoint
	}
	return nil
...
const file_proto_connector_v1_connector_proto_rawDesc = "" +
	...
	"\x06config\x18\x02 \x01(\fR\x06config\x12\x1f\n" +
	"\vresume_from\x18\x03 \x01(\fR\n" +
	"resumeFrom\"[\n" +
	"\x05Frame\x12\x18\n" +
	"\apayload\x18\x01 \x01(\fR\apayload\x12\x18\n" +
	"\arecords\x18\x02 \x01(\x03R\arecords\x12\x1e\n" +
	"\n" +
