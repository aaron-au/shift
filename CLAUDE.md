...
9. Flow model v2: a **step graph** with typed outcome edges — `onSuccess`/`onComplete` happy path, `onFailure` error handler (dead-letter). Linear source/ops/sink kept as sugar; both lower to one validated `Plan`. Engine `OpError` tags failures by step id; per-step-id telemetry. (ADR-0013). **Flow model v3 (ADR-0029) is built on top and is the current model**: the data path is an executable DAG — `tee`/`router` fan-out, `merge` fan-in (`concat` + spilling keyed `join`). v2 is the degenerate single-chain DAG and runs unchanged. Any validated topology executes — nested fan-outs and mixed fan-out/fan-in included — via `runner/internal/service/dag.go` (segments joined at tee/router/merge, `stream.Pipe` between stages).
...
11. Trigger & ingress: two planes — control (hub↔runner, metadata only: lease, config sync, execution reports) and data (ingress→runner, runner→source; **payload never touches the hub**). Webhooks are push triggers on a runner (`@webhook` source, async), reported to the hub as metadata. **Runners bind privately in every deployment (ADR-0038 supersedes ADR-0016 here)** — the sole public component is the optional DMZ-deployable **`shift-gateway`** (third app, `gatewayd`): TLS termination, caller auth, filtering/allowlists/rate limits, and routing to an available runner via ADR-0030 groups. It holds no DB connection, no KEK, no user secrets and no durable payload; explicitly **not** a WAF, not a plugin host, not a queue (no runner ⇒ 503). Absent by default: pull-only deployments carry zero inbound attack surface. (ADR-0016, ADR-0038)
...
13. Custom code: two tiers. **Tier 1 `starlark` is BUILT (ADR-0052)** — in-process `go.starlark.net`, fuel-metered, no I/O, no `load()` (so no supply chain at all), deterministic (at-least-once makes a varying transform a correctness bug), values adapted over `record.Value` rather than materialised, exact decimal arithmetic in-script, script errors redacted from hub-visible reports, and **off unless `SHIFT_ALLOW_CODE_STEPS=1`** — the sandbox bounds blast radius, not authority, and the hub is still open-access (RBAC = issue #16). Tier 2 `python` out-of-process (connector subprocess, wheels-only, signed bundles) stays deferred; `subflow` reserved. (ADR-0017, ADR-0052)
...
16. Connector operation model: a connector = **one canvas node**; the author picks a **verb** from a dropdown and the node's source/sink role follows the action's declared `direction` (never chosen up front). Side-effecting verbs are **config-driven sources** (path on the node, emit a status record); `put`-style verbs are the sink. Built-in **`@discard`** sink (connector-free, side-effect-free terminal, sink-only, auto-appended by the studio) lets a single-verb flow satisfy "happy path ends at a sink". Built-in **`@response`** sink + **synchronous run** (`Service.RunSync` + `POST /api/flows/run`) return a flow's output to the caller in the same HTTP response (NDJSON, bounded; runner-side, payload never at hub) — the sync direct-execution path. **Phase 2 (designed, deferred):** request-reply action shape (mid-flow call emitting a response, the remaining engine/SDK addition), verb/HTTP-status naming. (ADR-0024)
...
- **Logs are structured, on stdout (ADR-0046):** `pkg/shiftlog` for hub/runner; the gateway keeps its own copy so its `go.mod` stays dependency-free, and a conformance test holds the two to one schema. Every record carries `component`/`version`; lifecycle records carry a stable `event` name — alert on `event`, never on `msg`. Never log payload, secrets or tokens.
...
runner/     runnerd (M3a+M3b+M4b, done — see docs/dev/04-runner.md): flow docs → engine pipelines,
  ...
  internal/starlarkop                         `starlark` code step (ADR-0052/0017 tier 1): in-process,
                                              fuel-metered, no I/O, no load(), deterministic; values
                                              ADAPTED over record.Value (never materialised); exact
                                              decimal arithmetic in-script; OFF unless
                                              SHIFT_ALLOW_CODE_STEPS=1 (fail closed)
